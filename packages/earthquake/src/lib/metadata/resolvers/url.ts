...
import { resolve } from "@stryke/path/file-path-fns";
import { joinPaths } from "@stryke/path/join-paths";
import { isString } from "@stryke/type-checks/is-string";
import type { MetadataContext } from "../types/resolvers";

function isStringOrURL(icon: any): icon is string | URL {
  return isString(icon) || icon instanceof URL;
}
...
function resolveUrl(
  ...
  const joinedPath = joinPaths(pathname, url);
...
function resolveAbsoluteUrlWithPathname(
  ...
  if (trailingSlash && !resolvedUrl.endsWith("/")) {
    const isRelative = resolvedUrl.startsWith("/");
