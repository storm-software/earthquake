...
import { resolveArray, resolveAsArrayOrUndefined } from "../helpers";
...
import type { ResolvedTwitterMetadata, Twitter } from "../types/twitter";
...
function resolveAndValidateImage(
  ...
  const isRelativeUrl =
    typeof inputUrl === "string" && !/https?:\/\//.test(inputUrl);
...
function getFieldsByOgType(ogType: OpenGraphType | undefined) {
  if (!ogType || !(ogType in ogTypeToFields)) {
