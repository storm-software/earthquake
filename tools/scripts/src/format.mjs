...
try {
  ...
  let proc =
    $`pnpm exec storm-git readme --templates="tools/readme-templates" --project="@earthquake/monorepo"`.timeout(
      `${30 * 60}s`
    );
  ...
  proc =
    $`pnpm nx run-many --target=format --all --exclude="@earthquake/monorepo" --outputStyle=dynamic-legacy --parallel=5`.timeout(
      `${30 * 60}s`
    );
  proc.stdout.on("data", data => {
    echo`${data}`;
