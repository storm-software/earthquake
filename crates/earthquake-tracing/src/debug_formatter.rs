  ...
  N: for<'writer> FormatFields<'writer> + 'static,
{
  fn format_event(
    ...
    if let Some(mut action_meta) = action_meta {
      ...
      std::fs::create_dir_all(format!(".earthquake/logs/{session_id}")).ok();
      ...
      let log_filename: Arc<str> = if is_session_meta {
        format!(".earthquake/logs/{session_id}/meta.json").into()
      } else {
        format!(".earthquake/logs/{session_id}/logs.json").into()
      };
