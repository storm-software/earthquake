//! Earthquake binding crate: provides N-API (Node.js) facing helpers to create, start and
//! shutdown a tuned Tokio runtime (including WASM specific lifecycle helpers) used by the
//! earthquake build pipeline. The runtime sizing is customized to better match earthquake's
...
//   std::mem::transmute::<
//     &earthquake_plugin::TransformPluginContext<'_>,
//     &earthquake_plugin::TransformPluginContext<'_>,
//   >(ctx)
// }),
// Looks redundant
...
pub fn create_tokio_runtime(blocking_threads: Option<u32>) {
  ...
  let max_blocking_threads = blocking_threads
    .map(|v| v as usize)
    .or_else(|| {
      std::env::var("EARTHQUAKE_MAX_BLOCKING_THREADS").ok().and_then(|v| v.parse::<usize>().ok())
    })
  ...
  let rt = builder
    ...
    // earthquake puts a lot of blocking tasks in the worker threads rather than blocking_threads
